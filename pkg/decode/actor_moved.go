...
// ── Middle-generation unit_walking decoders (pv 20091103–20131222) ─────────────────────────────
// packet_unit_walking has moveStartTime after accessory and MoveData[6] instead of PosDir[3].
//   0x07F7: pv 20091103–20101123 — no robe, no maxHP/HP/isBoss (69 bytes)
//   0x0856: pv 20101124–20120220 — adds robe (71 bytes)
//   0x0914: pv 20120221–20131222 — adds maxHP+HP+isBoss (80 bytes)
// Source: packets_struct.hpp:758–830 (packet_unit_walking).

// ActorMoved_0x07F7 decodes a 0x07F7 packet (struct packet_unit_walking).
// Active: pv >= 20091103, < 20101124. Wire size: 69 bytes.
func ActorMoved_0x07F7(data []byte, packetver uint32) events.ActorMoved {
	...
	e.MoveStartTime = leU32(data, 29) // rAthena: moveStartTime
	e.Accessory2 = leU16(data, 33)    // rAthena: accessory2
	e.Accessory3 = leU16(data, 35)    // rAthena: accessory3
	e.Headpalette = leI16(data, 37)   // rAthena: headpalette
	e.Bodypalette = leI16(data, 39)   // rAthena: bodypalette
	e.HeadDir = leI16(data, 41)       // rAthena: headDir
	e.GUID = leU32(data, 43)          // rAthena: GUID
	e.GEmblemVer = leI16(data, 47)    // rAthena: GEmblemVer
	e.Honor = leI16(data, 49)         // rAthena: honor
	e.Virtue = leI32(data, 51)        // rAthena: virtue
	e.IsPKModeON = data[55]           // rAthena: isPKModeON
	e.Sex = data[56]                  // rAthena: sex
	e.MoveData = [6]byte(data[57:63]) // rAthena: MoveData
	e.XSize = data[63]                // rAthena: xSize
	e.YSize = data[64]                // rAthena: ySize
	e.Clevel = leI16(data, 65)        // rAthena: clevel
	e.Font = leI16(data, 67)          // rAthena: font
	return e
}

// ActorMoved_0x0856 decodes a 0x0856 packet (struct packet_unit_walking).
// Active: pv >= 20101124, < 20120221. Wire size: 71 bytes (adds robe).
func ActorMoved_0x0856(data []byte, packetver uint32) events.ActorMoved {
	...
	e.Bodypalette = leI16(data, 39)   // rAthena: bodypalette
	e.HeadDir = leI16(data, 41)       // rAthena: headDir
	e.Robe = leU16(data, 43)          // rAthena: robe (added pv >= 20101124)
	e.GUID = leU32(data, 45)          // rAthena: GUID
	e.GEmblemVer = leI16(data, 49)    // rAthena: GEmblemVer
	e.Honor = leI16(data, 51)         // rAthena: honor
	e.Virtue = leI32(data, 53)        // rAthena: virtue
	e.IsPKModeON = data[57]           // rAthena: isPKModeON
	e.Sex = data[58]                  // rAthena: sex
	e.MoveData = [6]byte(data[59:65]) // rAthena: MoveData
	e.XSize = data[65]                // rAthena: xSize
	e.YSize = data[66]                // rAthena: ySize
	e.Clevel = leI16(data, 67)        // rAthena: clevel
	e.Font = leI16(data, 69)          // rAthena: font
	return e
}

// ActorMoved_0x0914 decodes a 0x0914 packet (struct packet_unit_walking).
// Active: pv >= 20120221, < 20131223. Wire size: 80 bytes (adds maxHP+HP+isBoss).
func ActorMoved_0x0914(data []byte, packetver uint32) events.ActorMoved {
	...
	e.Bodypalette = leI16(data, 39)   // rAthena: bodypalette
	e.HeadDir = leI16(data, 41)       // rAthena: headDir
	e.Robe = leU16(data, 43)          // rAthena: robe
	e.GUID = leU32(data, 45)          // rAthena: GUID
	e.GEmblemVer = leI16(data, 49)    // rAthena: GEmblemVer
	e.Honor = leI16(data, 51)         // rAthena: honor
	e.Virtue = leI32(data, 53)        // rAthena: virtue
	...
	e.Clevel = leI16(data, 67)        // rAthena: clevel
	e.Font = leI16(data, 69)          // rAthena: font
	e.MaxHP = leI32(data, 71)         // rAthena: maxHP (added pv >= 20120221)
	e.HP = leI32(data, 75)            // rAthena: HP
	e.IsBoss = data[79]               // rAthena: isBoss
	return e
}
