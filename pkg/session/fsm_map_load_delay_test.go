...
// (default), this is <100ms. With MapLoadDelay=300ms, it should be >=250ms
// and <1s.
//
...
func TestConnect_MapLoadDelay_AppliedBeforeLoadEndAck(t *testing.T) {
	...
	tests := []struct {
		...
		maxElapsed time.Duration // upper bound (guards against unit confusion)
	}{
		{name: "no_delay", delay: 0, minElapsed: 0, maxElapsed: 100 * time.Millisecond},
		{name: "300ms_delay", delay: 300 * time.Millisecond, minElapsed: 250 * time.Millisecond, maxElapsed: 1 * time.Second},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if loadEndAckElapsed > tt.maxElapsed {
				t.Errorf("LoadEndAck arrived after %v, expected <= %v (delay=%v)",
					loadEndAckElapsed, tt.maxElapsed, tt.delay)
			}
		})
	}
}

// TestConnect_MapLoadDelay_CancelledByContext verifies that the MapLoadDelay
// sleep is interruptible by context cancellation. When ctx is cancelled during
// the delay, Connect should return promptly with ctx.Err() instead of waiting
// the full duration.
func TestConnect_MapLoadDelay_CancelledByContext(t *testing.T) {
	const pv = uint32(20180307)
	const aid = uint32(2000001)
	const sid1 = uint32(0xDEADBEEF)
	const sid2 = uint32(0xCAFEBABE)
	...
	mapScript := func(t *testing.T, conn net.Conn) {
		mustDrain(t, conn, 19)
		mustWrite(t, conn, buildZCAID(aid))
		// Send ZC_ACCEPT_ENTER to trigger the delay. We won't send anything
		// else — the FSM should abort during the sleep.
		mustWrite(t, conn, buildZCAcceptEnterWithPos(0x12345678, 150, 200, 3, 7))
		// Block forever — the FSM should cancel before reading anything.
		buf := make([]byte, 1)
		conn.Read(buf)
	}
	...
	server := ServerConfig{
		...
		MapLoadDelay: 5 * time.Second, // long delay; we'll cancel before it completes
	}
	creds := Credentials{Username: "test", Password: "pass", CharSlot: 0}
	...
	// Cancel the context after 200ms (well within the 5s delay).
	go func() {
	...
	t.Logf("Connect returned after %v with error: %v", elapsed, err)
}
