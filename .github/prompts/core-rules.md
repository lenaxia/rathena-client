...
### 1. Test-Driven Development (TDD) — README-LLM.md Rule 1
...
Every code change must include: multiple happy-path tests, multiple unhappy-path tests, edge cases, **fuzz tests for all bit-packing functions**, and **benchmarks verifying 0 allocs/op** on decode/encode functions. Unit tests alone are not sufficient.
...
Every non-trivial claim rests on assumptions. Unstated, unvalidated assumptions cause most bugs. This is doubly critical here because the semantic DB has hundreds of known errors — never trust it without GCC verification (Rules 9, 12).

**Mandatory protocol:**
- State every assumption explicitly before relying on it, with a confidence level (LOW/MEDIUM/HIGH) per Rule 10.
- Validate every assumption — read the source code, run the GCC preprocessor, run a test, check git history. Do not proceed on an assumption you have not verified.
- If you cannot validate an assumption, do not rely on it. Redesign so it is unnecessary, or ask the user.
- Record what proved each assumption (file path, GCC command output, test name).
...
**Never claim what the code does without reading it.** Do not describe behavior from memory, convention, or inference. Read the actual source, trace the actual path, confirm the actual behavior. Every assertion about packet structure, field names, or field types MUST cite a specific rAthena file:line or GCC preprocessor output (Rule 12).
