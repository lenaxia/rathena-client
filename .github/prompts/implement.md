...
4. For any packet-structure work, source field names, types, and order from rAthena only (Rule 6). Query the semantic DB via the `gokore-semantics` MCP server as a starting point, then verify against rAthena via the GCC preprocessor (`validation/`) before implementing (Rules 9, 12). Cite every claim.
